	...
	try {
		...
		await sendEmail(
			...
			OrderCreationTemplate({
				lang: locale,
				items: userCart.items.map((item)=>({...item, productItem: {...item.productItem, product: {...item.productItem.product, imageUrl: `https://${process.env.VERCEL_BRANCH_URL}${item.productItem.product.imageUrl}`}} })) as any,
				translation: t,
				order,
